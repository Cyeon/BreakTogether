...
void SoundMgr::Play(const wstring& _strKey)
{
	...
	m_pSystem->update();
	auto eChannel = SOUND_CHANNEL::SC_BGM;
	if (!ptSound->bLoop)
		eChannel = SOUND_CHANNEL::SC_EFFECT;
	m_pSystem->playSound(ptSound->pSound, nullptr, false, &m_pChannel[static_cast<UINT>(eChannel)]);
}

void SoundMgr::Pause(SOUND_CHANNEL _eChannel, bool _p)
{
	m_pChannel[static_cast<UINT>(_eChannel)]->setPaused(_p);
}
...
{
	auto iter = m_mapSod.find(_strKey);
